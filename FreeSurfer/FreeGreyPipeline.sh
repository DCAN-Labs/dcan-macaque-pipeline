...
for TXw in $Modalities; do
  ...
  if [ $TXw = "T1wN" ]; then
    ...
    SubjectID=${Subnum}N
    imcp $T1wNImage ${SubjectDIR}
    imcp $T1wNImageBrain ${SubjectDIR}
  fi
...
cat "$SubjectDIR"/"$SubjectID"/mri/transforms/eye.dat | sed "s/${SubjectID}/${SubjectID}_1mm/g" > "$SubjectDIR"/"$SubjectID"_1mm/mri/transforms/eye.dat # potential issue here.
